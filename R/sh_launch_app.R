...
#' @seealso
#'  \code{\link[shiny]{shinyApp}}
#' @rdname launch_basic_app
#' @export
#' @importFrom shiny shinyApp
launch_basic_app <- function(r_data_dir_chr,
                             credentials_tb = NULL,
                             shinyio_lgl = F) {
  if(shinyio_lgl){
    library(V8)
  ...
  shiny::shinyApp(
    make_basic_ui_fn(secure_lgl = !is.null(credentials_tb)),
    make_basic_server_fn(r_data_dir_chr = r_data_dir_chr,
                   credentials_tb = credentials_tb)
  )
}
