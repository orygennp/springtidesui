...
make_base_case_lup <- function(pa_x_params_ls,
...
dates_nms_chr_vec <- c("t0_prev",
                       #ti_col_chr_vec, ADD BACK IN WHEN PREVALENCE CALCULATED FOR i TIME POINTS
                      "tx_prev")
...
# tx_RR_tb <- RR_tb %>%
#   dplyr::slice(2) 
base_case_syd_RR_lup <- make_base_case_lup(pa_x_params_ls = pa_x_params_ls,
...
#   
#   
#   all_distress_tb <- readxl::read_xlsx(path_to_K10_data_chr,
#                                      range = "A3:J734")
# youth_distress_tb <- readxl::read_xlsx(path_to_K10_data_chr,
#                                        range = "L3:U734")
...
new_alt_results_ls <- update_results_ls(pa_x_params_ls_path_chr = pa_x_params_ls_path_chr,
...
# tx_RR_tb <- RR_tb %>%
#   dplyr::slice(2) 
# ## Make below a fn to do all intervening timepoints
...
```{r}
original_tb <- output_params_ls$sim_results_ls %>%
  purrr::map_dfr(~tibble::tibble(Tx = .x$tx_prev_any_common_mental_disorder_p_tl %>% sum(),
                                 T0 = .x$t0_prev_any_common_mental_disorder_p_tl %>% sum(),
                                 Tx_pop = .x$tx_20250101_p_tl %>% sum())) %>%
  dplyr::summarise_all(mean) %>%
  t()
comparisons_tb <- alt_results_ls %>% purrr::map_dfr(~ tibble::tibble(Springtides = .x$tx_prev_any_common_mental_disorder_p_tl %>% sum(),
`Springtides plus base case shock` = .x$TX_PREV_ANY_COMMON_MENTAL_DISORDER_P_TL_S1_EXCESS %>% sum(),
`Springtides plus background risk` = .x$SYD_BC_tx_prev_any_common_mental_disorder_p_tl %>% sum(),
`Springtides plus base case shock plus background risk` = .x$SYD_BC_TX_PREV_ANY_COMMON_MENTAL_DISORDER_P_TL_S1_EXCESS %>% sum(),
)
                                    ) %>%
  dplyr::summarise_all(mean) %>%
  t()
...
comparison_tb <- comparisons_tb %>%
  ...
  dplyr::mutate(`Implied health financing requriement` = .$`Annual QALY Difference (Springtides_Baseline)`*28033)

write.csv(comparison_tb,
