...
make_original_tb <- function(pa_x_params_ls_path_chr){
  pa_x_params_ls <- readRDS(pa_x_params_ls_path_chr)
pa_x_params_ls$sim_results_ls %>%
...
make_comparisons_tb <- function(results_ls){
  comparisons_tb <- results_ls %>% purrr::map_dfr(~ tibble::tibble(`Base case` = .x$tx_prev_any_common_mental_disorder_p_tl %>% sum(),
`Base case plus shock` = .x$TX_PREV_ANY_COMMON_MENTAL_DISORDER_P_TL_S1_EXCESS %>% sum(),
...
```{r echo=F, warning=F, message=F, eval=T}
original_tb <- make_original_tb(pa_x_params_ls_path_chr)
comparisons_tb <- make_comparisons_tb(results_ls = alt_results_ls) %>%
